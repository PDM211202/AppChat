...
io.on('connection', (socket) => {
  socket.on("join room from client to server", ({ room, username }) => {
    ...
    socket.emit("send message from server to client", createMessage(`Welcome to room ${room}`, "admin"));

    // gui cho cac client con lai
    socket.broadcast.to(room).emit("send message from server to client", createMessage(`${username} vua tham gia vao phong`, "admin"));
    ...
    socket.on("share location from client to server",
      ({ latitude, longitude }) => {
        ...
        const id = socket.id;
        const user = findUser(id);
        io.to(room).emit("share location from server to client", createMessage(linkLocation, user.username)) ;
      }
    )
